...
def ckey(mid, mtype, line): return f”{mid}*{mtype}*{line}”
...
ht = cur_period in (“H2”,“FT”,“HT”) or (cur_period==“H1” and (cur_min or 0)>=45)
if ht: result=“win” if not line_crossed else “lose”; fail=“Line crossed before HT” if result==“lose” else None
elif elapsed>65: result,fail=“lose”,“HT timeout”
...
el.innerHTML=` <div class="sr"> <div class="sc"><div class="sn" style="color:var(--green)">${data.goals||data.total_goals||0}</div><div class="sl">Goals</div></div> <div class="sc"><div class="sn" style="color:var(--blue)">${(data.snapshots||data.total_snapshots||0).toLocaleString()}</div><div class="sl">Snapshots</div></div> <div class="sc"><div class="sn" style="color:var(--yellow)">${data.obs||data.total_obs||0}</div><div class="sl">Observations</div></div> <div class="sc"><div class="sn" style="color:${(data.hit_rate||data.success_rate||0)>=55?'var(--green)':'var(--red)'}">${data.hit_rate||data.success_rate||0}%</div><div class="sl">Hit Rate</div></div> </div> <div class="tc"> <div class="card"> <div class="stit">Collection Progress</div> ${targets.map(t=>`
...
`).join('')} </div> <div class="card"> <div class="stit">Top Rules by Signals</div> ${(data.top_rules||[]).map(r=>`
...
1. Which rules look promising based on available data?
1. What patterns should we watch for?
1. Recommended next steps for data collection.
